  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(base + detail || 'Fail to call Scofield Assistance')
      }

      const reply: Message = {
        id: Date.now() + 1,
        role: 'assistant',
        content: data.reply ?? '(no responses)',
      }
    ...
    <main className="min-h-screen flex flex-col items-center justify-between p-4 bg-gray-50">
      <div className="w-full max-w-md flex flex-col gap-4">
        <header className="text-center">
          <h1 className="text-2xl font-bold mb-1">Scofield Assistance Chatbox</h1>
          <p className="text-gray-600 text-sm">
            Thanks for Gemini and Safari support
          </p>
        </header>
        ...
        <section className="flex-1 min-h-[320px] max-h-[480px] overflow-y-auto rounded-2xl border border-gray-200 bg-white p-3 space-y-2">
          {messages.length === 0 && (
            <p className="text-gray-400 text-sm text-center mt-12">
              start to send message, talk to Scofield assistant…
            </p>
          )}
          {messages.map((m) => (
            <div
          ...
          {loading && (
            <p className="text-gray-400 text-xs text-center mt-2">
              Scofield is stupid and he is thinking
            </p>
          )}
        </section>
